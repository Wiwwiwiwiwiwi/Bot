...
# ── رمزنگاری state (اختیاری) ──
# اگر STATE_SECRET تنظیم شود، توکن‌های Railway در فایل state رمزنگاری می‌شوند.
# کلید: یک رشته‌ی ۴۴ کاراکتری base64 (آدرس ۳۲ بایتی). با `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"`.
SECRET = os.environ.get("STATE_SECRET", "").strip()
_CIPHER = None
if SECRET:
    try:
        from cryptography.fernet import Fernet
        _CIPHER = Fernet(SECRET.encode())
    except Exception:
        _CIPHER = None
        print("⚠️ STATE_SECRET داده شد ولی رمزنگاری فعال نشد (cryptography نصب نیست؟)")

# ── تنظیمات ────────────────────────────────────────────
BOT_TOKEN = os.environ.get("BOT_TOKEN", "")
SCRIPT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "scripts")
RAILWAY_URL = "https://backboard.railway.com/graphql/v2"
REPO = os.environ.get("REPO", "Kolkolz/3xui-multi-region")
BRANCH = os.environ.get("BRANCH", "main")

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
logger = logging.getLogger(__name__)

# ── ذخیره وضعیت کاربران (در حافظه + فایل برای بقا بعد از ری‌استارت) ─
# user_id → {
...
# پایداری: فایل state (ولوم Railway روی /data) — با هر تغییر ذخیره می‌شود
STATE_FILE = os.environ.get("STATE_FILE", "/data/state.json")
LOG_DIR = os.environ.get("LOG_DIR", "/data/logs")

# احراز هویت: ALLOWED_UID = "123,456" — خالی = همه مجازند (پیش‌فرض)
ALLOWED_UID = {int(x.strip()) for x in os.environ.get("ALLOWED_UID", "").split(",") if x.strip()}


def _enc_state(users):
    """کپی users با توکن‌های رمزنگاری‌شده (اگر _CIPHER فعال باشد)، وگرنه همان."""
    if not _CIPHER:
        return users
    out = {}
    for uid, u in users.items():
        accs = {}
        for name, a in u.get("accounts", {}).items():
            a = dict(a)
            if "token" in a and a["token"]:
                a["token_e"] = _CIPHER.encrypt(a["token"].encode()).decode()
                a.pop("token", None)
            accs[name] = a
        u2 = dict(u)
        u2["accounts"] = accs
...
def _dec_state(users):
    """برعکس _enc_state — توکن‌ها را از token_e رمزگشایی و در token بگذار."""
    if not _CIPHER:
        return users
    for uid, u in users.items():
        for a in (u.get("accounts") or {}).values():
            te = a.pop("token_e", "")
            if te:
                try:
                    a["token"] = _CIPHER.decrypt(te.encode()).decode()
                except Exception:
                    a["token"] = ""
    return users
...
def save_state():
    """ذخیره USERS روی دیسک (اتمی — اول tmp بعد rename). توکن‌ها رمزنگاری می‌شوند."""
    try:
        os.makedirs(os.path.dirname(STATE_FILE) or ".", exist_ok=True)
        tmp = STATE_FILE + ".tmp"
        with open(tmp, "w") as f:
            json.dump(_enc_state(USERS), f, ensure_ascii=False)
...
def load_state():
    """بارگذاری USERS از دیسک در استارت‌آپ — توکن‌های رمزنگاری‌شده را رمزگشایی می‌کند."""
    global USERS
    try:
        with open(STATE_FILE) as f:
            USERS = _dec_state(json.load(f))
    except Exception:
        USERS = {}


def log_session(acc_name, event, detail=""):
    """لاگ سشن — خط JSON با timestamp (برای دیباگ)."""
    try:
        os.makedirs(LOG_DIR, exist_ok=True)
        with open(os.path.join(LOG_DIR, "session.log"), "a") as f:
            f.write(json.dumps({
...
def is_allowed(update):
    """وایتدلیست UID — اگر ALLOWED_UID ست شده باشد فقط همان‌ها دسترسی دارند."""
    if not ALLOWED_UID:
        return True
    return bool(update.effective_user) and update.effective_user.id in ALLOWED_UID


def require_auth(func):
    """دکوراتور — گارد دسترسی مشترک (کاهش تکرار is_allowed در هندلرها).

    برای هندلرهای تلگرام؛ در صورت عدم مجوز، پیام «⛔ دسترسی نداری.» را می‌فرستد.
    """
...
# ریجن موردنظر هر سرویس (طبق راهنمای مرحله ۳ — کاربر در داشبورد همین‌ها را می‌گذارد)
SERVICE_REGIONS = {
    "xui-nl": "ams",
    "xui-sg": "sin",
    "xui-us-va": "iad",
...
def ensure_volumes(token, pid, env_id, services):
    """ساخت ولوم /etc/x-ui برای سرویس‌های بدون ولوم (best-effort).

    قبلاً ولوم در deploy.py ساخته می‌شد، ولی با SKIP_REGION=1 سرویس هنوز
    ریجن ندارد و ولوم با ریجن فرضی ساخته می‌شد. حالا بعد از تنظیم ریجن
    توسط کاربر، اینجا ساخته می‌شود.
    """
    made = []
    for name, region in SERVICE_REGIONS.items():
        ...
        try:
            ...
            if d.get("data") and d["data"].get("volumeCreate"):
                made.append(name)
        except Exception as e:
            logger.warning("volume %s: %s", name, str(e)[:120])
    return made
...
async def run_script_async(script, env_extra, msg, title, emoji="⏳",
                           timeout=1500, show_tail=True):
    """اجرای اسکریپت به صورت async با نمایش زنده روی پیام.

    مثل run_rotate: پیام هر ~۴ ثانیه آپدیت می‌شود حتی بدون خروجی،
    سقف زمانی دارد و پروسه را kill می‌کند. برمی‌گرداند (lines, returncode).
    """
    env = dict(os.environ)
    env.update(env_extra)
    script_path = os.path.join(SCRIPT_DIR, script)
    ...
    try:
        while time.time() < deadline:
            ...
            if now - last_edit > 4:
                last_edit = now
                body = f"⏱ {int(now - started)} ثانیه"
                if show_tail and tail:
                    body += "\n\n" + pre("\n".join(tail))
                try:
                    await msg.edit_text(
                        card(title, body, emoji=emoji,
                             footer="⏳ در حال اجرا…"),
                        parse_mode=HTML,
                    )
                except Exception:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ۲.۵) UUID برای بازسازی لینک بعد از چرخش (دکمه «🔗 ساخت لینک جدید»)
    if acc and acc.get("awaiting_uuid"):
        acc["awaiting_uuid"] = False
        save_state()
        if re.match(r"^[0-9a-fA-F-]{36}$", text):
            await make_links(update, context, text)
        else:
            await update.effective_message.reply_text(
                "❌ UUID معتبر نیست! فرمت درست:\n"
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ── پاکسازی پروژه‌های تستی ──
        if data.startswith("cleanup_del:"):
            pid = uncb(data)
            await q.answer("در حال حذف…")
            try:
                gql(acc["token"], 'mutation($id: String!){ projectDelete(id: $id) }', {"id": pid})
                await q.message.edit_text(
                    card("پاکسازی", f"🗑 پروژه <code>{esc(pid[:8])}</code> حذف شد.", emoji="🧹"),
                    parse_mode=HTML,
                )
                log_session(acc.get("name", "?"), "cleanup_del", pid)
            except Exception as e:
                await q.message.edit_text(
                    f"❌ <code>{esc(str(e)[:120])}</code>", parse_mode=HTML)
            return
        if data == "cleanup_all":
            await q.answer()
            await q.message.reply_text(
                "🧹 مطمئنی <b>همه</b> پروژه‌های تستی حذف شوند؟\n(غیرقابل بازگشت)",
                parse_mode=HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ بله، همه", callback_data="cleanup_all_yes")],
                    [InlineKeyboardButton("❌ نه", callback_data="cleanup_all_no")],
                ]),
            )
            return
        if data == "cleanup_all_yes":
            await q.answer("در حال حذف…")
            try:
                d = gql(acc["token"],
                        'query($wid: String!){ projects(workspaceId: $wid) { edges { node { id name } } } }',
                        {"wid": acc["workspace_id"]})
                pids = [e["node"]["id"] for e in d["data"]["projects"]["edges"]
                        if e["node"]["name"] == "3xui-multi-region"]
                done = 0
                for pid in pids:
                    try:
                        ...
                        pass
                await q.message.edit_text(
                    card("پاکسازی", f"🧹 {done} پروژه حذف شد.", emoji="🧹"),
                    parse_mode=HTML,
                )
                log_session(acc.get("name", "?"), "cleanup_all", f"{done}/{len(pids)}")
            except Exception as e:
                await q.message.edit_text(
                    f"❌ <code>{esc(str(e)[:120])}</code>", parse_mode=HTML)
            return
        if data == "cleanup_all_no":
            await q.answer("لغو شد")
            try:
                await q.message.delete()
            except Exception:
...
async def tcp_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    ...
    # ساخت لینک جدید بعد از چرخش
    if data == "proxy_links":
        acc["awaiting_uuid"] = True
        save_state()
        await q.answer("UUID را بفرست 🔗")
        await q.message.reply_text(
            "🔗 <b>UUID کلاینت</b> را بفرست تا لینک‌های جدید\n"
            "با پروکسی‌های تازه ساخته شوند.",
            parse_mode=HTML,
        )
        return
    ...
    if new_map:
        ...
        save_state()
        log_session(acc.get("name", "?"), "rotate_done", f"regions={list(new_map)}")
        body = tcp_body(acc, extra="✅ پروکسی‌ها چرخیدند!")
        footer = ("لینک‌های قبلی نامعتبر شدند ❗\n"
                  "دکمه زیر را بزن و UUID را بفرست تا لینک‌های جدید بسازم.")
        await msg.edit_text(
            card("چرخش کامل شد", body, emoji="🌐", footer=footer),
            parse_mode=HTML,
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔗 ساخت لینک جدید", callback_data="proxy_links"),
            ]]),
        )
    else:
        await msg.edit_text(
...
async def setup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ۲) ساخت سرویس‌ها — async با نمایش زنده (ربات قفل نمی‌شود)
        lines, rc = await run_script_async(
            "deploy.py",
            {
                "RAILWAY_TOKEN": token,
            ...
            msg, "مرحله ۲ — ساخت سرویس‌ها", emoji="📡", timeout=1500,
        )
        low = "\n".join(lines).lower()
        if "TIMEOUT" in lines:
            await msg.edit_text(
                card("خطا در ساخت سرویس‌ها",
                     "⏱ بیشتر از حد مجاز طول کشید — بعداً دوباره /setup بزن.", emoji="❌"),
                parse_mode=HTML,
            )
            return
        if rc != 0 or "service creation limit" in low or "25 services per day" in low:
        ...
        domains = extract_lines("\n".join(lines), ["دامنه: https://"])
        panel_urls = []
        for d in domains:
        ...
        save_state()
        log_session(acc.get("name", "?"), "setup_done", f"project={pid}")
    except Exception as e:
        log_session(acc.get("name", "?"), "setup_error", str(e)[:200])
        await msg.edit_text(
            f"❌ خطا: <code>{esc(str(e)[:200])}</code>",
            parse_mode=HTML,
        )
...
async def continue_setup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        lines, rc = await run_script_async(
            ...
            msg, "مرحله ۴ — اتصال نودها", emoji="🔗", timeout=900,
        )
        out = "\n".join(lines)
        ...
        lines, rc = await run_script_async(
            ...
            msg, "مرحله ۵ — اینباند VLESS + Reality", emoji="🛡️", timeout=900,
        )
        out = "\n".join(lines)
        ...
        lines, rc = await run_script_async(
            ...
            msg, "مرحله ۶ — TCP Proxy + Host ها", emoji="🌐", timeout=1800,
        )
        out = "\n".join(lines)
        ...
        # ولوم‌ها — حالا که ریجن تنظیم شده (P2-7: قبلاً در deploy.py با SKIP_REGION رد می‌شد)
        vols = ensure_volumes(token, pid, env_id, services)
        if vols:
            await msg.edit_text(
                card(
                    "TCP Proxy",
                    "✅ TCP Proxy ساخته شد!\n" + (pre(out[-400:]) if out else "")
                    + f"\n\n💾 ولوم‌ها: {vols}",
                    emoji="🌐",
                ),
                parse_mode=HTML,
        ...
        acc["domains"] = domains
        save_state()
        log_session(acc.get("name", "?"), "continue_done", f"proxies={len(proxy_map)}")
    except Exception as e:
        log_session(acc.get("name", "?"), "continue_error", str(e)[:200])
        await msg.edit_text(
            f"❌ خطا: <code>{esc(str(e)[:200])}</code>",
            parse_mode=HTML,
        )
...
async def make_client(update: Update, context: ContextTypes.DEFAULT_TYPE, name: str):
    ...
    msg = await update.effective_message.reply_text(
        card("ساخت کلاینت", f"👤 در حال ساخت «{esc(name)}»…", emoji="👤"),
        parse_mode=HTML,
    )
    lines, rc = await run_script_async(
        "xui-client-create.py",
        {
            "PANELS": panels_env,
            "MAIN_PANEL": "xui-nl",
            "CLIENT_NAME": name,
        ...
        msg, "ساخت کلاینت", emoji="👤", timeout=300, show_tail=False,
    )
    out = "\n".join(lines)
    sub_https = ""
    uuid_val = ""
...
def cleanup_kbd(projs):
    rows = []
    for p in projs:
        rows.append([
            InlineKeyboardButton(f"🗑 {p['svc']} سرویس — {p['createdAt'][:10]}",
                                 callback_data=cb("cleanup_del", p["id"])),
        ])
    rows.append([
        InlineKeyboardButton("🧹 حذف همه", callback_data="cleanup_all"),
        InlineKeyboardButton("🔙 بازگشت", callback_data="cleanup_back"),
    ])
    return InlineKeyboardMarkup(rows)
...
async def cleanup_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """لیست پروژه‌های تستی (3xui-multi-region) اکانت فعال + حذف از داخل چت."""
    uid = update.effective_user.id
    acc = get_acc(USERS.get(uid))
    if not acc or "token" not in acc:
        await update.effective_message.reply_text(
            "⚠️ اول توکن اکانت را ثبت کن (👥 اکانت‌ها).",
            parse_mode=HTML,
        )
        return
    try:
        d = gql(acc["token"],
                'query($wid: String!){ projects(workspaceId: $wid) { edges { node { id name createdAt } } } }',
                {"wid": acc["workspace_id"]})
        projs = []
        for e in d["data"]["projects"]["edges"]:
            p = e["node"]
            if p["name"] != "3xui-multi-region":
                continue
            try:
                dd = gql(acc["token"],
                         'query($id: String!){ project(id: $id) { services { edges { node { id } } } } }',
                         {"id": p["id"]})
                n = len(dd["data"]["project"]["services"]["edges"])
            except Exception:
                n = -1
            projs.append({"id": p["id"], "name": p["name"],
                          "svc": n, "createdAt": p.get("createdAt", "")[:10]})
        if not projs:
            await update.effective_message.reply_text(
                "🧹 پروژه تستی (3xui-multi-region) پیدا نشد.",
                parse_mode=HTML,
            )
            return
        lines = [f"  • <code>{p['id'][:8]}</code> — {p['svc']} سرویس ({p['createdAt']})"
                 for p in projs]
        await update.effective_message.reply_text(
            card(
                "پاکسازی پروژه‌های تستی",
                f"{len(projs)} پروژه پیدا شد:\n\n" + "\n".join(lines)
                + "\n\n⚠️ با حذف، سرویس‌ها و داده‌هایشان هم پاک می‌شوند.",
                emoji="🧹",
            ),
            parse_mode=HTML,
            reply_markup=cleanup_kbd(projs),
        )
    except Exception as e:
        await update.effective_message.reply_text(
            f"❌ خطا: <code>{esc(str(e)[:150])}</code>",
            parse_mode=HTML,
        )


async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    """هندلر خطاهای پیش‌بینی‌نشده — لاگ + اطلاع به کاربر (اگه شد)."""
    logger.error("خطای هندل نشده: %s", context.error, exc_info=context.error)
    try:
        if update and getattr(update, "effective_message", None):
            await update.effective_message.reply_text(
                "❌ خطای غیرمنتظره:\n" + esc(str(context.error))[:150],
                parse_mode=HTML,
            )
    except Exception:
        pass
...
def main():
    ...
    # بارگذاری state ذخیره‌شده (ولوم /data) — بقا بعد از ری‌استارت
    load_state()
    logger.info("state: %d کاربر بارگذاری شد", len(USERS))

    # تأخیر استارتاپ — به Railway فرصت می‌دهد نمونه قبلی را REMOVE کند
    # تا 409 (Conflict) در getUpdates رخ ندهد
    delay = float(os.environ.get("STARTUP_DELAY", "20"))
    if delay > 0:
        print(f"⏳ {int(delay)} ثانیه صبر (حذف نمونه قبلی)…")
        time.sleep(delay)
